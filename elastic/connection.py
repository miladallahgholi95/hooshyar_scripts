...
class ESIndex:
    ...
    def bulk_insert_documents(self, documents, do_parallel=True):
        ...
        if do_parallel:
            deque(helpers.parallel_bulk(self.CLIENT, generate_docs_method(documents), thread_count=8, chunk_size=200,
                                        request_timeout=3000), maxlen=0)
        else:
            helpers.bulk(self.CLIENT, generate_docs_method(documents), chunk_size=200, request_timeout=3000)
